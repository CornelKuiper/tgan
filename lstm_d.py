...
class Model(object):
	...
	def test(self, i):
		...
		for ix in trange(0, data_size, self.batch_size):
			...
			D_cost_test, D_acc_test, D_acc_t_test= self.session.run([self.D_loss, self.D_accuracy, self.D_accuracy_total], feed_dict={self.x: batch_x, self.y_: batch_y})
	...
	def train(self):
		...
		for i in trange(self.start, self.end):
			...
			embedding = np.load("D:/ML_datasets/data_glove/shuffled/train{}.npy".format(i%8))
			labels = np.load("D:/ML_datasets/data_glove/shuffled/labels{}.npy".format(i%8))
...
if __name__ == '__main__' :
	...
	model.restore("D:/ML_datasets/logs_d/conv_mean/no_dropout/1", 150)
